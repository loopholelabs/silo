...
func TestMigratorSimpleCowSparse(t *testing.T) {
	...
	assert.NoError(t, err)
	cow := modules.NewCopyOnWrite(sourceStorageMem, overlay, blockSize, &modules.CopyOnWriteConfig{SharedBase: true})
	...
	destStorage := sources.NewMemoryStorage(size)
	destCow := modules.NewCopyOnWrite(sourceStorageMem, destStorage, blockSize, &modules.CopyOnWriteConfig{SharedBase: true})
