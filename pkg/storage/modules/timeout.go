...
func NewTimeout(prov storage.StorageProvider, timeoutRead time.Duration, timeoutWrite time.Duration, timeoutClose time.Duration, timeoutFlush time.Duration) *Timeout {
	return &Timeout{
		prov:         prov,
...
func (i *Timeout) SendEvent(event_type storage.EventType, event_data storage.EventData) []storage.EventReturnData {
