...
func (p *TestProtocolBandwidth) SendPacketWriter(dev uint32, id uint32, length uint32, header []byte, data func(w io.Writer) error) (uint32, error) {
	return p.proto.SendPacketWriter(dev, id, length, header, data)
}
