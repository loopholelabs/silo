...
func TestMigrator(t *testing.T) {
	...
	mig := storage.NewMigrator(sourceDirty, locker, unlocker)

	destStorageLat := modules.NewArtificialLatency(destStorage, 0, 10*time.Millisecond)

	// TODO: Currently this is done in a single go with no feedback.
	mig.MigrateTo(destStorageLat, blockSize, sourceMonitor.GetNextBlock)
